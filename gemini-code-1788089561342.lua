-- ============================================
-- @set9p | SILENT AIM + MULTI-POINT VISIBLE CHECK + AUTO TRIGGER + ESP + BHOP
-- ============================================
...
local Settings = {
    ...
    AutoTrigger = false,
    MaxTargetWorldDistance = 150, -- Абсолютный лимит дальности до цели (исключает спавн)
...
-- ============================
-- ПЕРЕРАБОТАННАЯ ПРОВЕРКА ВИДИМОСТИ (MULTI-POINT)
-- ============================
local function isDirectlyVisible(targetPart)
    ...
    -- Проверяем ключевые точки модели для повышенной точности
    local checkParts = {
        targetPart,
    ...
    local origin = char.HumanoidRootPart.Position + Vector3.new(0, 2, 0)
    ...
    for _, part in ipairs(checkParts) do
        if part and part:IsA("BasePart") then
            ...
            if (origin - targetPos).Magnitude <= Settings.MaxTargetWorldDistance then
...
local function GetClosestPlayer()
    ...
    for _, v in pairs(plrs) do
        ...
        if worldDist > Settings.MaxTargetWorldDistance then
            continue
        end
...
RunService.RenderStepped:Connect(function()
    ...
    if lockedTarget and lockedTarget.Parent then
        ...
        if hum and hum.Health > 0 then
            if isDirectlyVisible(lockedTarget) then
                isStillValid = true
            end
        end
...
CombatTab:CreateSlider({
    ...
    Callback = function(v) Settings.MaxTargetWorldDistance = v end
})
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Скрипт запущен: многоточечная проверка видимости активирована!",
    Duration = 3,
    Image = 4483362458,
})