-- ============================================
-- SWILL | FULL SCRIPT (WALLCAM + AIM + JUMP TRAIL + ESP FIXED)
-- ============================================
...
-- JUMP TRAIL (СЛЕД ПРИ ПРЫЖКЕ / ДВИЖЕНИИ)
local jumpTrailEnabled = false
local trailColor = Color3.fromRGB(255, 0, 80)
...
-- ============================
-- JUMP TRAIL (СЛЕД ПРИ ПРЫЖКЕ)
-- ============================
local function setupJumpTrail(char)
    local hrp = char:FindFirstChild("HumanoidRootPart")
    local hum = char:FindFirstChildOfClass("Humanoid")
    if not hrp or not hum then return end

    hum.StateChanged:Connect(function(_, newState)
        if newState == Enum.HumanoidStateType.Jumping and jumpTrailEnabled then
            task.spawn(function()
                for i = 1, 10 do
                    ...
                    p.Color = espRGB and getRainbowColor() or espColor
                    p.Parent = Workspace
                    
                    task.spawn(function()
                        local startT = tick()
                        while tick() - startT < 0.5 do
                            local alpha = (tick() - startT) / 0.5
                            p.Transparency = alpha
                            p.Size = Vector3.new(1 - alpha, 1 - alpha, 1 - alpha)
...
if LocalPlayer.Character then setupJumpTrail(LocalPlayer.Character) end
LocalPlayer.CharacterAdded:Connect(setupJumpTrail)
...
MiscTab:CreateToggle({
    Name = "Jump Trail (След при прыжке)",
    CurrentValue = false,
    Flag = "JumpTrailToggle",
    Callback = function(v) jumpTrailEnabled = v end
})
...
Rayfield:Notify({
    Title = "Готово",
    Content = "WallCam возвращен, трейсеры удалены, ESP исправлен!",
    Duration = 3,
    Image = 4483362458,
})